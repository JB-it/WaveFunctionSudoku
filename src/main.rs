...
async fn main() {
    ...
    loop {
        ...
        if is_mouse_button_down(MouseButton::Left) && !hovered {
            let (mouse_x, mouse_y) = (
                (mouse_position().0 / 100.0) as usize,
                (mouse_position().1 / 100.0) as usize,
            );
            match sudoku.get_cell(mouse_x, mouse_y) {
                Field::Collapsed { num: _ } => {}
                Field::Superposition { states } => {
                    let mx =
                        ((mouse_position().0 - mouse_x as f32 * 100.0) / (100.0 / 3.0)) as usize;
                    let my =
                        ((mouse_position().1 - mouse_y as f32 * 100.0) / (100.0 / 3.0)) as usize;
...
fn draw_selected(sudoku: &Sudoku) {
    ...
    match sudoku.get_cell(mouse_x, mouse_y) {
        Field::Collapsed { num: _ } => {
            draw_rectangle(
                mouse_x as f32 * 100.0,
                mouse_y as f32 * 100.0,
                100.0,
                100.0,
                GRAY,
            );
        ...
        Field::Superposition { states } => {
            ...
            if states[index] {
                draw_rectangle(
                    mouse_x as f32 * 100.0 + mx.floor() * (100.0 / 3.0),
                    ...
                    for sx in 0..3 {
                        for sy in 0..3 {
                            ...
                            draw_rectangle_lines(
                                x as f32 * 100f32 + sx as f32 * 100f32 / 3f32,
